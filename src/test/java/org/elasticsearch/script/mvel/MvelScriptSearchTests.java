...
public class MvelScriptSearchTests extends ElasticsearchIntegrationTest {
    ...
    @Test
    public void testMvelScriptFilter() throws Exception {
        createIndex("test");
    ...
    public void testScriptFieldUsingSource() throws Exception {
        ...
        SearchResponse response = client().prepareSearch()
                ...
                .addScriptField("s_obj1", "mvel", "_source.obj1", null)
                .addScriptField("s_obj1_test", "mvel", "_source.obj1.test", null)
                .addScriptField("s_obj2", "mvel", "_source.obj2", null)
                .addScriptField("s_obj2_arr2", "mvel", "_source.obj2.arr2", null)
                .execute().actionGet();
    ...
    public void testCustomScriptBoost() throws Exception {
        ...
        SearchResponse response = client().search(searchRequest()
                ...
                .query(functionScoreQuery(termQuery("test", "value"))
                        .add(ScoreFunctionBuilders.scriptFunction("doc['num1'].value").lang("mvel"))))
        ).actionGet();
        ...
        response = client().search(searchRequest()
                .searchType(SearchType.QUERY_THEN_FETCH)
                .source(searchSource().explain(true).query(functionScoreQuery(termQuery("test", "value"))
                        .add(ScoreFunctionBuilders.scriptFunction("Math.pow(doc['num1'].value, 2)").lang("mvel"))))
        ).actionGet();
        ...
        response = client().search(searchRequest()
                .searchType(SearchType.QUERY_THEN_FETCH)
                .source(searchSource().explain(true).query(functionScoreQuery(termQuery("test", "value"))
                        .add(ScoreFunctionBuilders.scriptFunction("Math.max(doc['num1'].value, 1d)").lang("mvel"))))
        ).actionGet();
        ...
        response = client().search(searchRequest()
                .searchType(SearchType.QUERY_THEN_FETCH)
                .source(searchSource().explain(true).query(functionScoreQuery(termQuery("test", "value"))
                        .add(ScoreFunctionBuilders.scriptFunction("doc['num1'].value * _score").lang("mvel"))))
        ).actionGet();
        ...
        response = client().search(searchRequest()
                .searchType(SearchType.QUERY_THEN_FETCH)
                .source(searchSource().explain(true).query(functionScoreQuery(termQuery("test", "value"))
                        .add(ScoreFunctionBuilders.scriptFunction("param1 * param2 * _score").param("param1", 2).param("param2", 2).lang("mvel"))))
        ).actionGet();
    ...
    @Test
    public void testMvelEmptyParameters() throws Exception {
        createIndex("test");
        index("test", "type1", "1", jsonBuilder().startObject().field("myfield", "foo").endObject());
        refresh();

        client().prepareUpdate("test", "type1", "1").setScriptLang("mvel").setScript("ctx[\"_source\"][\"myfield\"]=\"bar\"", ScriptService.ScriptType.INLINE)
                .execute().actionGet();
        refresh();
